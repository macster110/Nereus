...
def species_month(key: str, title: str, sfx: str) -> Question:
    ...
    return Question(
        ...
        nereus=lambda c, p: sql_df(c, """
            SELECT s.doc_id, s.deployment_ref, d.t_start, d.t_end
...
return <r doc="{{$d/Id}}" dep="{{$d/DataSource/DeploymentId}}" s="{{$x/Start}}" e="{{$x/End}}"/>
        ...
        json=lambda p: {
            "select": [
                {"op": "=", "operands": ["Detections/OnEffort/Detection/SpeciesId", p[tsn]], "optype": "binary"},
            ...
            "enclose": 1, "namespaces": 0, "species": {}},
    )
...
Q.append(Question(
    ...
    nereus=lambda c, p: sql_df(c, """
        ...
        FROM nereus.detection d JOIN nereus.detection_set s ON s.id = d.set_id
        WHERE d.on_effort AND s.deployment_ref = %s""", (p["deployment"],)),
    xquery=lambda p: f"""
<Result>{{
...
Q.append(Question(
    ...
    nereus=lambda c, p: sql_df(c, """
        ...
        WHERE d.on_effort AND d.species_tsn = %s
          ...
          AND dep.location::geometry && ST_MakeEnvelope(%s, %s, %s, %s, 4326)""",
...
Q.append(Question(
    ...
    nereus=lambda c, p: sql_df(c, """
        SELECT s.doc_id, s.deployment_ref, s.effort_start, s.effort_end, k.call, k.granularity
...
return <r doc="{{$d/Id}}" dep="{{$d/DataSource/DeploymentId}}" s="{{$d/Effort/Start}}"
    ...
    json=lambda p: {
        "select": [{"op": "=", "operands": ["Detections/Effort/Kind/SpeciesId", p["tsn"]], "optype": "binary"}],
        "return": ["Detections/Id", "Detections/DataSource/DeploymentId",
...
Q.append(Question(
    ...
    nereus=lambda c, p: sql_df(c, """
        SELECT s.deployment_ref AS dep, date_trunc('day', d.t_start, 'UTC') AS day, count(*) AS n
        FROM nereus.detection d JOIN nereus.detection_set s ON s.id = d.set_id
        WHERE d.on_effort AND d.species_tsn = %s
        GROUP BY 1, 2""", (p["tsn"],)),
    xquery=lambda p: f"""
...
Q.append(Question(
    ...
    nereus=lambda c, p: sql_df(c, """
        SELECT s.doc_id, s.deployment_ref, ST_Y(dep.location::geometry) AS lat,
               ST_X(dep.location::geometry) AS lon, s.effort_start, s.effort_end,
               (SELECT count(*) FROM nereus.detection d WHERE d.set_id = s.id AND d.on_effort) AS n
        FROM nereus.detection_set s LEFT JOIN nereus.deployment dep ON dep.id = s.deployment_id"""),
    xquery=lambda p: """
<Result>{
